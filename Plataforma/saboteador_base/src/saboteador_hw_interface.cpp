#include <saboteador_base/saboteador_hw_interface.h>
...
{
    SaboteadorHWInterface::SaboteadorHWInterface(ros::NodeHandle &nh, urdf::Model *urdf_model)
    ...
    { 
        ...
        //sub_left_encoder_ticks_ = nh_.subscribe("encoder_ticks", 10, &SaboteadorHWInterface::encoderTicksCallback, this);
        sub_encoder_ticks_ = nh.subscribe("encoder_ticks", 10, &SaboteadorHWInterface::encoderTicksCallback, this);
        // Initialize the hardware interface
        init(nh_, nh_);
    }

 
    bool SaboteadorHWInterface::init(ros::NodeHandle &root_nh, ros::NodeHandle &robot_hw_nh)
    {
        ROS_INFO("Initializing saboteador Hardware Interface ...");
        ...
        ROS_INFO("... Done Initializing saboteador Hardware Interface");
        return true;
    }

    void SaboteadorHWInterface::read(const ros::Time& time, const ros::Duration& period)
    {
    ...
    void SaboteadorHWInterface::write(const ros::Time& time, const ros::Duration& period)
    {
        ros::Duration elapsed_time = period;
        // Write to robot hw
    ...
    void SaboteadorHWInterface::loadURDF(const ros::NodeHandle &nh, std::string param_name)
    {
    ...
    void SaboteadorHWInterface::printState()
    {
    ...
    std::string SaboteadorHWInterface::printStateHelper()
    {
        std::stringstream ss;
    ...
    std::string SaboteadorHWInterface::printCommandHelper()
    {
        std::stringstream ss;
        std::cout.precision(15);
    ...
    void SaboteadorHWInterface::encoderTicksCallback(const diffbot_msgs::Encoder::ConstPtr& msg)
    {
    ...
    double SaboteadorHWInterface::normalizeAngle(double &angle) const
    {
    ...
    double SaboteadorHWInterface::linearToAngular(const double &distance) const
    {
        return distance / wheel_diameter_ * 2.0;
    }

    double SaboteadorHWInterface::angularToLinear(const double &angle) const
    {
